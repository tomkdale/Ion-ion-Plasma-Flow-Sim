//--------------------------------------------------
//Function calculates new voltages and new E fields in the R and Z direction for every point on edge grid for grounded case
//-------------------------------------------------

#ifndef _VOLTAGESGROUND
#define _VOLTAGESGROUND

//CALCULATES RED VALUES FOR right end grounded case
__global__ void calc_voltagesRedGrounded(double*  newVolt, double* oldVolt, double* densityP, double* densityN, double dR, double dZ, bool* converged) {
	...
	if (i == 0 && j == 0) {//left inner corner
		...
		newVolt[grid(i, j, numEdgeZ)] = 0.0; // point left
	}
	else {
		//if interior point
...
}//end of calc_voltagesRedZeroGradient




 //CALCULATES BLACK VALUES FOR Grounded case
__global__ void calc_voltagesBlackGrounded(double*  newVolt, double* oldVolt, double* densityP, double* densityN, double dR, double dZ, bool* converged) {
	int i = blockIdx.y * blockDim.y + threadIdx.y;//r position index
	int j = blockIdx.x * blockDim.x + threadIdx.x;//z position index
